...
class ConvReluNorm(layers.Layer):
	...
	def call(self, x, x_mask):
		print(f"ConvReluNorm layer")
		x_org = x
		for i in range(self.n_layers):
			print(f"layer {i} x: {x}, {x.shape}")
			x = self.conv_layers[i](x * x_mask)
			print(f"layer {i} conv layer x: {x}, {x.shape}")
			x = self.norm_layers[i](x)
			print(f"layer {i} norm layer x: {x}, {x.shape}")
			x - self.relu_drop(x)
			print(f"layer {i} relu dropout x: {x}, {x.shape}")
		print(f"x_org: {x_org}, {x_org.shape}")
		x = x_org + self.proj(x)
		print(f"residual + projection x: {x}, {x.shape}")
		return x * x_mask
...
class MultiHeadAttention(layers.Layer):
	...
	def attention(self, query, key, value, mask=None):
		...
		print(f"b {b}, d {d}, t_s {t_s}, t_t {t_t}")
		...
		print(f"query: {query}, {query.shape}")
		print(f"key: {key}, {key.shape}")
		print(f"value: {value}, {value.shape}")

		scores = tf.linalg.matmul(query, tf.transpose(key, [0, 1, 3, 2])) / math.sqrt(self.k_channels)
		...
		if self.window_size is not None:
			...
			print(f"key_relative_embeddings: {key_relative_embeddings}, {key_relative_embeddings.shape}")
			rel_logits = self._matmul_with_relative_keys(
				query, key_relative_embeddings
			)
			print(f"rel_logits: {rel_logits}, {rel_logits.shape}")
			rel_logits = self._relative_position_to_absolute_position(
				rel_logits
			)
			print(f"rel_logits: {rel_logits}, {rel_logits.shape}")
			scores_local = rel_logits / math.sqrt(self.k_channels)
			print(f"scores_local: {scores_local}, {scores_local.shape}")
		...
		if self.window_size is not None:
			...
			print(f"relative_weights: {relative_weights}, {relative_weights.shape}")
			value_relative_embeddings = self._get_relative_embeddings(
				self.emb_rel_v, t_s
			)
			print(f"value_relative_embeddings: {value_relative_embeddings}, {value_relative_embeddings.shape}")
	...
	def _matmul_with_relative_values(self, x, y):
		...
		print(f"(matmul w/ relative values) ret {ret}, {ret.shape}")
		return ret
	...
	def _matmul_with_relative_keys(self, x, y):
		...
		print(f"(matmul w/ relative keys) ret {ret}, {ret.shape}")
		return ret
	...
	def _get_relative_embeddings(self, relative_embeddings, length):
		...
		print(f"slice_start_position {slice_start_position}")
		print(f"slice_end_position {slice_end_position}")
		print(f"relative_embeddings {relative_embeddings.shape}")
		if pad_length > 0:
			...
			print(f"padded_relative_embeddings: {padded_relative_embeddings}, {padded_relative_embeddings.shape}")
		else:
			padded_relative_embeddings = relative_embeddings
		used_relative_embeddings = padded_relative_embeddings[
			:, slice_start_position:slice_end_position
		]
		print(f"used_relative_embeddings: {used_relative_embeddings}, {used_relative_embeddings.shape}")
	...
	def _relative_position_to_absolute_position(self, x):
		...
		print(f"x_flat {x_flat}, {x_flat.shape}")
		x_flat = tf.pad(
		...
		print(f"x_final {x_final}, {x_final.shape}")
		return x_final
...
class Encoder(layers.Layer):
	...
	def call(self, x, mask):
		print(tf.expand_dims(mask, 2).shape)
		print(tf.expand_dims(mask, -1).shape)
		...
		for i in range(self.n_layers):
			...
			print(f"layer {i} attn_layers y: {y}, {y.shape}")
			y = self.drop(y)
			x = self.norm_layers1[i](x + y)
			print(f"layer {i} norm_layers 1 x: {x}, {x.shape}")
			y = self.ffn_layers[i](x, mask)
			print(f"layer {i} ffn_layers y: {y}, {y.shape}")
			y = self.drop(y)
			x = self.norm_layers2[i](x + y)
			print(f"layer {i} norm_layers 2 x: {x}, {x.shape}")
		x = x * mask
		print(f"x: {x}, {x.shape}")
		return x
...
class TextEncoder(keras.Model):
	...
	def call(self, x, x_lengths, spk=None):
		...
		print(f"initial input lengths: {x_lengths}, {x_lengths.shape}")
		x = self.emb(x) * math.sqrt(self.n_channels)
		print(f"x embedding: {x}, {x.shape}")
		...
		print(f"prenet output: {x}, {x.shape}")
		if self.n_spks > 1:
			x = tf.concat(
				[
					x, 
			...
			print(f"x after n_spks > 1: {x}, {x.shape}")
		x = self.encoder(x, x_mask)
		print(f"encoder output x: {x}, {x.shape}")
		mu = self.proj_m(x) * x_mask
		print(f"mu {mu}, {mu.shape}")

		x_dp = tf.identity(x)
		log_w = self.proj_w(x_dp, x_mask)
		print(f"log_w {log_w}, {log_w.shape}")
