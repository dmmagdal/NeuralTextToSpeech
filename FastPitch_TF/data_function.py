...
from common.utils import load_wav, load_filepaths_and_text
...
class BetaBinomialInterpolator:
	...
	def __init__(self, round_mel_len_to=100, round_text_len_to=20):
		self.round_text_len_to = round_mel_len_to
	...
	def __call__(self, w, h):
		bw = self.round(w, to=self.round_text_len_to)
...
def estimate_pitch(wav, mel_len, method="pyin", normalize_mean=None,
		normalize_std=None, n_formats=1):
	...
	if method == 'pyin':
		snd, sr = librosa.load(wav) # Load sound and sample resolution
		...
		pitch_mel = tf.expand_dims(
			tf.convert_to_tensor(pitch_mel), axis=0
		)
		pitch_pad = [[0, mel_len - pitch_mel.shape[1]]]
		pitch_mel = tf.pad(pitch_mel, pitch_pad)
...
class TTSDataset:
	...
	# 2) normalize text and converts them to sequences of one-hot
	# vectors
	# 3) computes mel-spectrogram from audio files
	def __init__(self,
			dataset_path,
			audiopaths_and_text,
			text_cleaners,
		...
		if len(self.audiopaths_and_text[0]) > expected_columns:
			print(
				"WARNING: Audiopaths file has more columns than expected"
			)
	...
	def __getitem__(self, index):
		...
		energy = tf.norm(tf.cast(mel, dtype=tf.float32), ord=2, axis=0)
	...
	def get_mel(self, filename):
		if not self.load_mel_from_disk:
			...
			if sampling_rate != self.stft.sampling_rate:
				raise ValueError(
					"{} SR doesnt match target {} SR".format(
				...
				melspec = tf.squeeze(melspec, axis=0)
			else:
				melspec = tf.io.read_file(filename)
	...
	def get_prior(self, index, mel_len, text_len):
		...
		if self.betabinomial_tmp_dir is not None:
			...
			if cached_fpath.is_file():
				return tf.io.read_file(cached_fpath)
		...
		if self.betabinomial_tmp_dir is not None:
			...
			tf.io.write_file(attn_prior, cached_fpath)
	...
	def get_pitch(self, index, mel_len=None):
		...
		if self.load_pitch_from_disk:
			pitchpath = fields[0]
			pitch = tf.io.read_file(pitchpath)
		...
		if self.pitch_tmp_dir is not None and not cached_fpath.is_file():
			cached_fpath.parent.mkdir(parents=True, exist_ok=True)
			tf.io.write_file(pitch_mel, cached_fpath)
...
class TTSCollater:
	...
	def __call__(self, batch):
		...
		text_padded = tf.zeros(
			(len(batch), max_input_length), dtype=tf.int64
		)
		...
		mel_padded = tf.zeros(
			(len(batch), num_mels, max_target_len), dtype=tf.float32
		)
		output_lengths = tf.zeros((len(batch)), dtype=tf.int64)
		for i in range(len(ids_sorted_decreasing)):
			mel = batch[ids_sorted_decreasing[i]][1]
			mel_padded[i, :, :mel.shape[1]] = mel
			output_lengths[i] = mel.shape[1]

		n_formnts = batch[0][3].shape[0]
		pitch_padded = tf.zeros(
			(mel_padded.shape[0], n_formants, mel_padded.shape[2]),
			dtype=batch[0][3].dtype
		)
		energy_padded = tf.zeros_like(pitch_padded[:, 0, :])
		...
		if batch[0][5] is not None:
			speaker = tf.zeros_like(input_lengths)
			for i in range(len(ids_sorted_decreasing)):
				speaker_id[i] = batch[ids_sorted_decreasing[i]][5]
		else:
			speaker = None

		attn_prior_padded = tf.zeroes(
			(len(batch), max_target_len, max_input_length)
		)
		for i in range(len(ids_sorted_decreasing)):
			prior = batch[ids_sorted_decreasing[i]][6]
			attn_prior_padded[i, :prior.shape[0], :prior.shape[1]] =\
				prior

		# Count number of items - characters in text
		len_x = [x[2] for x in batch]
		len_x tf.convert_to_tensor(len_x)

		audiopaths = [batch[i][7] for i in ids_sorted_decreasing]

		return (
			text_padded, input_lengths, mel_padded, output_lengths,
			len_x, pitch_padded, energy_padded, speaker, 
