...
def _epxand_dimension(m):
	...
	if text.endswith(tuple(_dimension_key.keys())):
		if text[-2].isdigit():
			text = "{} {}".format(
				text[:-1], _dimension_key[text[-1:]]
			)
		elif text[-3].isdigit():
			text = "{} {}".format(
				text[:-2], _dimension_key[text[-2:]]
			)
	return text
...
def normalize_letters_and_numbers(text):
	...
	text = re.sub(
		_letters_and_numbers_re, _expand_letters_and_numbers, text
	)
	return text
