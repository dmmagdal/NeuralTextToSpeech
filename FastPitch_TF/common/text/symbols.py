...
text that has been run through Unidecode. For other data, you can 
...
# Prepend "@" to ARPAbet symbols to ensure uniqueness (some are the
# same as uppercase letters):
...
def get_symbols(symbol_set='english_basic'):
	if symbol_set == 'english_basic':
		...
		raise Exception(
			'{} symbol set does not exist'.format(symbol_set)
		)
