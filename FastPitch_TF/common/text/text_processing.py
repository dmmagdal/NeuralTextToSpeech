...
# Regular expression separating words enclosed in curly braces for
# cleaning
...
class TextProcessing(object):
	...
	def symbols_to_sequence(self, symbols):
		return [
			self.symbol_to_id[s] for s in symbols 
			if s in self.symbol_to_id
		]


	def arpabet_to_sequence(self, text):
		return self.symbols_to_sequence(
			['@' + s for s in text.split()]
		)


	def get_arpabet(self, words):
		arpabet_suffix = ''
		...
		if len(word) > 2 and word.endswith("'s"):
			...
			if arpabet is None:
				arpabet = self.get_arpabet(words[:-2])
				arpabet_suffix = ' Z'
		elif len(word) > 1 and word.endswith("s"):
			arpabet = cmudict.lookup(word)
			if arpabet is None:
				arpabet = self.get_arpabet(words[:-1])
				arpabet_suffix = ' Z'
		else:
			arpabet = cmudict.lookup(word)
	...
	def encode_text(self, text, return_all=False):
		...
		text_arpabet = ''
		if self.p_arpabet > 0:
			if self.handle_arpabet == 'sequence':
				...
				raise Exception(
					"{} handle_arpabet is not supported"\
						.format(self.handle_arpabet)
				)
