...
class CMUDict:
	...
	'''
	def __init__(self, file_or_path=None, heteronyms_path=None, 
			keep_ambiguous=True):
		self._entries = {}
		self.heteronyms = {}
		if file_or_path is not None:
			self.initialize(
				file_or_path, homonyms_path, keep_ambiguous
			)


	def initialize(self, file_or_path, homonyms_path, 
			keep_ambiguous=True):
		...
		if not keep_ambiguous:
			entries = {
				word: pron for word, pron in entries.items() 
				if len(pron) == 1
			}
		...
		if heteronyms_path is not None:
			with open(heteronyms_path, encoding='utf-8') as f:
				self.heteronyms = [l.strip() for l in f]
	...
	def download(self):
		...
		try:
			...
			urllib.request.urlretrieve(
				url, filename='cmudict/cmudict-0.7b'
			)
		except:
