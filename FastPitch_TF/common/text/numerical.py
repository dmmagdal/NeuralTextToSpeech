...
_magnitudes = [
	'trillion', 'billion', 'million', 'thousand', 'hundred', 'm', 'b', 
	't'
]
_magnitudes_key = {
...
_measurements_key ={
	'f': 'fahrenheit', 'c': 'celsius', 'k': 'thousand', 'm': 'meters'
}
_currency_key = {
	'$': 'dollar', '£': 'pound', '€': 'euro', '₩': 'won'
}
...
def _expand_currency(m):
	...
	if magnitude is not None and magnitude.lower() in _magnitudes:
		...
		return "{} {} {}".format(
			_expand_hundreds(quantity), magnitude, currency+'s'
		)
...
def _expand_measurement(m):
	...
	measurement = _magnitudes_key[measurement.lower()]
...
def _expand_number(m):
	...
	if number > 1000 < 10000 and (number // 100) and (number % 1000 != 0):
