 ...
 * Training Notes:
	 * Original authors trained their model for 1 million steps at batch size 16 for 13,000 to 16,000 samples across 8x Nvidia 2070Ti cards (8GB VRAM each). 
		 ...
		 * UPDATE 1 (09/06/23): After training on my 2060 SUPER at 4 batches for the specified number of steps, I found the model produced was less than robust (the model produced at the end of training only generated static noise). Since then, I've acquired new hardware (a GPU server with 3x Nvidia P100 16GB Tesla cards) and have begun single device (only 1 card) training at batch size 16. 
			 * A few things around multi GPU single device training on the server. I added a new parameter to the `params.py` file called `dist_batch_size`. This is to refer to the local batch size that is served on each GPU. The global batch size is the local batch size x the number of available/assigned GPUs on the device. 
			 * Despite single GPU training on the P100 being able to handle batch size of 16, the multi GPU configuration would OOM the first/master GPU if the `dist_batch_size` is set to 16 (making the global batch size 48). This is probably from overhead when the resulting weights and losses are aggregated. A `dist_batch_size` of 8 would also OOM, so that is why the value is set at 4 at the time of this update. 
