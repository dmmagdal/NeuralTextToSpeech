...
def main():
	...
	parser.add_argument('--multi_gpu_single_device', action='store_true', default=False,
		help='train model on multi-gpu single device')
	args = parser.parse_args()

	if args.multi_gpu_single_device:
...
def distribute_train(args):
	###################################################################
	# Similar to single gpu single device training.
	###################################################################
	...
	print("-" * 72)
	print(f"Initializing training with {num_devices} cards on this device...")
	print("-" * 72)
	...
	# Distribute the datasets across devicse with the given
	...
	with strategy.scope():
		...
		# Build the model. Don't forget to include batch dim in the 
		# input shape.
		model.summary()
		...
		# Logic for resuming training from latest checkpoint or train 
		# from scratch.
		if args.resume_training and os.path.exists(checkpoint_dir) and len(os.listdir(checkpoint_dir)) > 0:
			latest_checkpoint = os.path.join(
				checkpoint_dir,
			...
			# No previous checkpoint exits. Create checkpoint directory
			# and start at the first epoch.
			os.makedirs(checkpoint_dir, exist_ok=True)
			latest_checkpoint = None
			initial_epoch = 0
	...
	for epoch in range(initial_epoch, epochs):
		...
		for train in train_dataset:
			...
			strategy.run(
				step, 
				args=(
					train, model, loss, params, epoch_loss_avg, True
				)
			)
