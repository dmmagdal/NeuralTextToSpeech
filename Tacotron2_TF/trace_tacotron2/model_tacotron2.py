...
from layers_tacotron2 import ConvNorm, LinearNorm
...
class Encoder(nn.Module):
	...
	def forward(self, x, input_lengths):
		print(f"encoder inputs shape pre ConvBlocks: {x.size()}")
		for conv in self.convolutions:
			x = F.dropout(F.relu(conv(x)), 0.5, self.training)
		print(f"encoder data shape post ConvBlocks: {x.size()}")

		# Transpose because LSTM takes channels last. 
		x = x.transpose(1, 2)

		print(f"encoder data post transpose shape: {x.size()}")
